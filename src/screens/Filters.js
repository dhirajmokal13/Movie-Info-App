...
const Filters = ({ route, navigation }) => {
    ...
    const handleChangeType = (value) => {
        if (value === "none") {
            setFilterDetails({ ...filterDetails, Type: false });
        } else {
            setFilterDetails({ ...filterDetails, Type: value });
        }
    }

    const handleChangeYear = (value) => {
        if (value === "none") {
            setFilterDetails({ ...filterDetails, Year: false });
        } else {
            setFilterDetails({ ...filterDetails, Year: value });
        }
    }
        ...
        <SafeAreaView className="flex-1 items-center mt-[8vh]">
            <View className="flex flex-row">
                <View className="relative w-[40vw] mr-[2.8vw] border border-gray-300 bg-gray-300 px-4 rounded-md">
                    <Picker className="relative w-[45vw] bg-gray-300 py-[3vh] px-5vw" onValueChange={handleChangeType} selectedValue={filterDetails.Type || "none"}>
                ...
                <View className="relative w-[40vw] ml-[2.8vw] border border-gray-300 bg-gray-300 px-4 rounded-md">
                    <Picker className="relative w-[45vw] bg-gray-300 py-[3vh] px-5vw" onValueChange={handleChangeYear} selectedValue={filterDetails.Year || "none"}>
                        <Picker.Item label="None" value="none" />
